...
const uint32_t IPU_TILE_INSTANCES_WITH_REPAIR = 0x4c0; // 1216
const uint32_t IPU_CHIP_TILE_INSTANCES_NO_REPAIR = 0x500; // 1280
const uint32_t IPU_CHIP_TILE_INSTANCES_WITH_REPAIR = 0x4c0; // 1216
...
const uint32_t TILE_INCOMING_MUX__RESET0 = 0x500; // 1280
...
// CSR Supervisor: $INCOMING_MUXPAIR
...
// CSR Supervisor: $INCOMING_FORMAT
...
// CSR Supervisor: $INCOMING_DCOUNT
...
enum TileRunMode {
  ...
  TRUNM_EXCEPTED = 2,
  TRUNM_REPEATING = 3,
  TRUNM_EXECUTE_INJECT = 4, // executing an injected instruction from TDI.$DBG_IEXEC
  TRUNM_SUSPENDED_NWE = 5,
  TRUNM_SUSPENDED_WE = 6,
  TRUNM_EXCHANGE_SEND = 7,
  TRUNM_DELAY = 8, // entered from a delay instruction
  TRUNM_WAIT_EXCHANGE = 9,
  TRUNM_WAIT_ANS = 10, // sync ANS?
  TRUNM_WAIT_WORKERS = 11, // sync LOCAL?
  TRUNM_WAIT_ISYNCACK = 12, // sync INTERNAL?
  TRUNM_WAIT_ESYNCACK = 13, // sync EXTERNAL_N/GSN?
};
...
// presumably defines the valid arguments for the "sync" instruction
enum TileSyncZone {
